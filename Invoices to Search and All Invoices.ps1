...
if ($vpnok.ConnectionStatus -eq 'connected')
    {
        Copy-Item -Path 'C:\Pastel18\CIRCE20A\Documents\PDF\*.pdf' -Destination "C:\Search Invoices" -Recurse -force
        Move-Item -Path 'C:\Pastel18\CIRCE20A\Documents\PDF\*.pdf' -Destination "\\wserver\wmarine\customers\_All Invoices & Credit Notes" -Force
     }
