<# Warren Marine - 
    Report - Prepaid group but trip was cancelled or they did not arrive
#>
$status = Test-Connection wserver
if ($status.statuscode -eq 0) {
    $file = Test-Path -path '\\wserver\wmarine\Booking Reports\BookingQueries*.xlsx'
    if ($file -eq $true) { 
        $path = '\\wserver\wmarine\Booking Reports\'
        #$path = 'C:\Test\'
        $file = Get-ChildItem -Path $path -Name 'BookingQueries*.xlsx'
        $insheet = $path + $file
        ...
        Get-ChildItem -Path $src2\BookingQueries*.xlsx | Move-Item -Destination $dest2 -Force
      
        $dest1 = '\\wserver\wmarine\Booking Reports\Julia'
    
        Get-ChildItem -Path $path\BookingQueries*.xlsx | Move-Item -Destination $dest1 -Force
