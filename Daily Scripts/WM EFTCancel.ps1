<# Warren Marine - 
    Report - EFT but Cancel for yesterday (if any).
#>
...
if ($status.statuscode -eq 0) {
    ...
    if ($file -eq $true) { 
        ...
        $insheet = $path + $file
        $file2 = "EFT_Cancel $(get-date -f yyyyMMdd-HHmm).xlsx"
        $outfile = $path + $file2
        ...
        #>
        Write-EventLog -LogName MyPowerShell -Source "WM" -EntryType Information -EventId 10 -Message "EFTCancel script completed"
    ...
    Guy-SendGmail "Connection to WServer does not exists!" "Script - EFTCancel" 
