<# Warren Marine - 
    Report - EFT but Did Not Arrive for yesterday (if any).
#>
...
if ($status.statuscode -eq 0) {
    ...
    if ($file -eq $true) { 
        ...
        $insheet = $path + $file
        $file2 = "EFT_Did Not Arrive $(get-date -f yyyyMMdd-HHmm).xlsx"
        $outfile = $path + $file2
        ...
        #$datefilter = "2019/03/04"                                     #Specific date
        ...
        #Status filter
        $xlfilter = "Did not arrive"
        $range3.AutoFilter(12, $xlfilter)
        #Payment type filter (EFT means it has been invoiced by bookkeeper)
        $xlfilter = "EFT"
        ...
        #>
        Write-EventLog -LogName MyPowerShell -Source "WM" -EntryType Information -EventId 10 -Message "EFTDidNotArrive script completed"

    }
    else {
        Guy-SendGmail "EFTDidNotArrive script failed." "Check connection to Server"
        Write-EventLog -LogName MyPowerShell -Source "WM" -EntryType Error -EventId 30 -Message "EFTDidNotArrive Script failed"
