...
if ($status.statuscode -eq 0) {
    $file = Test-Path -path '\\wserver\kiosk\invoices\Drumbeat\preparation\DrumbeatBookingDetails Arrived*.xlsx'
    if ($file -eq $true) { 
        $path = '\\wserver\Kiosk\invoices\Drumbeat\Preparation\'
        #$path = 'C:\Test\'
        $file = Get-ChildItem -Path $path -Name 'DrumbeatBookingDetails Arrived*.xlsx'
        $a = $path + $file
        ...
        Get-ChildItem -Path $src1\DrumbeatBookingDetails Arrived*.xlsx | Move-Item -Destination $dest1 -Force
...
else { Guy-SendGmail "Connection to WServer does not exists!" "WM Durmbeat" 
